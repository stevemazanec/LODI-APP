$(document).ready(function () {
    ...
    function addMarkers() {
        ...
        //Loops through each roofer and adds them to the map using their latitude and longitude
        $.get("/api/roofing", function (data) {
            for (var i = 0; i < data.length; i++) {
                var lat = data[i].latitude;
                var long = data[i].longitude;

                //Adds a marker indicating the roofer's location, along with their name
                var marker = L.marker([lat, long]).addTo(mymap);
                marker.bindPopup(data[i].name).openPopup();

                //Adds a circle indicating the radius the roofer works, along with their name
    ...
    var handleDisplayReviews = function (event) {
        ...
        $("#roofing-review-modal").modal("toggle");


        // display all reviews
        API.getReviews(currentId).then(function (result) {
            $("#roofer-modal-title").text("All reviews for " + currentname);
            $("#roofer-review-div").empty();

            for (var i = 0; i < result.length; i++) {
                var currentDate = result[i].createdAt.substring(5, 10) + "-" + result[i].createdAt.substring(0, 4);
                $("#roofer-review-div").append("<p>Rating: " + result[i].rating + "</p>");
                $("#roofer-review-div").append("<p>Date: " + currentDate + "</p>");
                $("#roofer-review-div").append("<p>Comment: " + result[i].comment + "</p>");
                $("#roofer-review-div").append("<p>--------------------------------------</p>");
            }
        });
    };
