$(document).ready(function () {
    ...
    function addMarkers() {
        ...
        //Loops through each painter and adds them to the map using their latitude and longitude
        $.get("/api/painting", function (data) {
            for (var i = 0; i < data.length; i++) {
                var lat = data[i].latitude;
                var long = data[i].longitude;

                //Adds a marker indicating the painter's location, along with their name
                var marker = L.marker([lat, long]).addTo(mymap);
                marker.bindPopup(data[i].name).openPopup();

                //Adds a circle indicating the radius the painter works, along with their name
